...
const RestaurantCard = ({ restur, categoryIndex, newcategoryIndex }) => {
  const [count, setcount] = useState(1);
  const [limit, setLimit] = useState(6 * count);
  ...
  const handleShowMoreImages = () => {
    if (limit <= restur[categoryIndex].restaurantList.length) {
      setLimit(limit + 6);
      setcount(count + 1);
    }
  };
  ...
  return (
    <>
      {restur[categoryIndex]?.restaurantList &&
      restur[categoryIndex].restaurantList.length > 0 ? (
        restur[categoryIndex].restaurantList
          .slice(0, limit)
          .map((restaurant, index) => (
            <div key={index} className="col-4 pb-3">
              ...
              >
                ...
                restur[categoryIndex].restaurantList.length > 0 ? (
                  <div
                    className="more text-center p-5"
                    onClick={handleShowMoreImages}
                  >
                    <h1 className="" style={{ color: "red" }}>
                      +
                      {restur[categoryIndex].restaurantList.length - 6 * count}
                      More
                    </h1>
                  </div>
                ) : (
                ...
                <div className="card-body p-0">
                  ...
                  <div className="d-flex">
                    {restaurant.food_types.slice(0, 3).map((type, index) => (
                  ...
                  <div className="d-flex justify-content-between">
                    ...
                    {restaurant.price_for_two !== "" ? (
                      <p>₹{restaurant.price_for_two} FOR TWO</p>
                    ) : (
                      <p>--</p>
                    )}
                  </div>
